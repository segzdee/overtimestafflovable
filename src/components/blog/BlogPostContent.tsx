            ...
            <div className="flex flex-wrap gap-1.5">
              {post.tags.map((tag: string) => (
                <Link
                  key={tag}
                  to={`/blog?tag=${tag}`}
                  className="bg-gray-100 text-gray-800 text-xs px-2 py-0.5 rounded-full hover:bg-gray-200 transition-colors flex items-center"
                >
                  <Tag className="inline-block h-2.5 w-2.5 mr-1" />
                  {tag}
                </Link>
              ))}
            </div>
