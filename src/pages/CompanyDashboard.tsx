    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4">
          <h2 className="text-xs sm:text-sm font-medium text-gray-500">Current Month Shifts</h2>
          <div className="flex items-center mt-2">
            <Calendar size={16} className="text-teal-600 mr-2 flex-shrink-0" />
            <span className="text-xl sm:text-2xl font-bold">28</span>
          </div>
        </div>
        
        <div className="bg-white rounded-lg shadow p-4">
          <h2 className="text-xs sm:text-sm font-medium text-gray-500">Staff Requested</h2>
          <div className="flex items-center mt-2">
        ...
        <div className="bg-white rounded-lg shadow p-4">
          <h2 className="text-xs sm:text-sm font-medium text-gray-500">Monthly Spend</h2>
          <div className="flex items-center mt-2">
            <DollarSign size={16} className="text-green-600 mr-2 flex-shrink-0" />
            <span className="text-xl sm:text-2xl font-bold">$8,450</span>
          </div>
        </div>

        <div className="bg-white rounded-lg shadow p-4">
          <h2 className="text-xs sm:text-sm font-medium text-gray-500">Pending Requests</h2>
          <div className="flex items-center mt-2">
            <ClipboardList size={16} className="text-orange-600 mr-2 flex-shrink-0" />
            <span className="text-xl sm:text-2xl font-bold">2</span>
          </div>
        </div>
      </div>

      {/* Upcoming Shifts and Pending Requests */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base sm:text-lg font-medium">Upcoming Shifts</h2>
            <button className="text-xs sm:text-sm bg-teal-600 text-white px-2 py-1 sm:px-3 sm:py-1 rounded whitespace-nowrap">
              + Request New Shift
            </button>
          </div>
                  ...
                  <tr key={shift.id}>
                    ...
                    <td className="px-3 py-3 whitespace-nowrap">
                      <div className="flex items-center">
                        <span className="text-xs sm:text-sm text-gray-900">{shift.staffConfirmed}/{shift.staffNeeded}</span>
                        <div className="w-12 sm:w-16 h-2 bg-gray-200 rounded ml-2">
                          <div 
              ...
              <div key={shift.id} className="border rounded-lg p-3 shadow-sm">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center mt-2">
                      <span className="text-xs text-gray-900">Staff: {shift.staffConfirmed}/{shift.staffNeeded}</span>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base sm:text-lg font-medium">Pending Staff Requests</h2>
            <button className="text-xs sm:text-sm text-teal-600">View All</button>
          </div>
          <div className="space-y-4">
            {pendingRequests.map(request => (
              <div key={request.id} className="border-b pb-4 last:border-0">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-500">{request.date} • {request.time}</p>
                  <div className="flex items-center mt-1">
                    <Users size={14} className="text-gray-400 mr-1" />
                    <span className="text-xs sm:text-sm text-gray-600">{request.staffNeeded} needed</span>
                  </div>
                  <div className="mt-2 flex flex-col xs:flex-row space-y-2 xs:space-y-0 xs:space-x-2">
            ...
            <button className="w-full text-xs sm:text-sm text-center text-teal-600 py-2 border border-dashed border-teal-300 rounded-lg hover:bg-teal-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base sm:text-lg font-medium">Favorite Staff</h2>
            <button className="text-xs sm:text-sm text-teal-600">View All</button>
          </div>
            ...
            {favoriteStaff.map(staff => (
              <div key={staff.id} className="border-b pb-3 last:border-0">
                <div className="flex flex-col xs:flex-row xs:justify-between xs:items-start">
                  ...
                  <button className="text-xs bg-teal-600 text-white px-2 py-1 rounded mt-2 xs:mt-0 touch-target">
                    Request
                  </button>
                </div>
              </div>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base sm:text-lg font-medium">Partner Agencies</h2>
            <button className="text-xs sm:text-sm text-teal-600">View All</button>
          </div>
          <div className="space-y-3">
            {topAgencies.map(agency => (
              <div key={agency.id} className="border-b pb-3 last:border-0">
                <div>
                  <h3 className="font-medium text-sm sm:text-base truncate">{agency.name}</h3>
                  <div className="grid grid-cols-3 gap-2 mt-2">
                    <div className="text-center">
                      <div className="text-xs text-gray-500">Staff</div>
                      <div className="font-medium text-xs sm:text-sm">{agency.staffProvided}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-xs text-gray-500">Reliability</div>
                      <div className="font-medium text-xs sm:text-sm">{agency.reliability}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-xs text-gray-500">Response</div>
                      <div className="font-medium text-xs sm:text-sm">{agency.responseTime}</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base sm:text-lg font-medium">Recent Invoices</h2>
            <button className="text-xs sm:text-sm text-teal-600">View All</button>
          </div>
          <div className="space-y-3">
      ...
      <div className="text-center mt-6">
        <Button
          ...
          className="bg-teal-600 text-white flex items-center gap-2 mx-auto"
        >
